 ...
 *
 * Runtime:
 * - Baileys: @itsliaaa/baileys (latest)
 *
 * Created by Nixel
...
import { generateWAMessageFromContent, prepareWAMessageMedia } from '@itsliaaa/baileys';
                ...
                return new Promise((resolve, reject) => {
                        ...
                        try {
                                if (!Buffer.isBuffer(videoBuffer) || !videoBuffer.length) {
                                        return fail(new Error('الفيديو ما هو صالح أو فاضي'));
                                }
                                ...
                                outputStream.on('end', async () => {
                                        try {
                                                ...
                                                if (!output.length) {
                                                        return fail(new Error('الناتج فاضي — تحقق من الصيغة أو توقيت الفيديو'));
                                                }
...
class ButtonV2 extends BaseBuilder {
        ...
        async build(jid, { ...options } = {}) {
                let _thumbnail = null;
                if (this._image) {
                        const imgBuf = Buffer.isBuffer(this._image) ? this._image : await Toolkit.fetchBuffer(this._image, {}, { silent: true });
                        if (imgBuf && imgBuf.length > 0) {
                                _thumbnail = await sharp(imgBuf)
